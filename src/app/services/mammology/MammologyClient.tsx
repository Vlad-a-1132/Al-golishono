...
const whyItems = [
  ...
  "Консультации онколога и детского онколога",
  "Профилактика и раннее выявление патологий",
  "Удобная запись и приём в Голицыно",
];

const symptoms = [
  "уплотнения или болезненность в молочной железе",
  "выделения из соска",
  "изменение формы или кожи груди",
  "профилактический осмотр и скрининг",
  "наблюдение при доброкачественных образованиях",
  "консультация по результатам обследований",
];

const helpsWith = [
  "оценка жалоб и факторов риска",
  "подбор плана обследований молочных желез",
  "консультация онколога при показаниях",
  "диспансерное наблюдение",
  "приём детского онколога (по показаниям)",
  "объяснение результатов УЗИ и других исследований",
];

const diagnostics = [
  "УЗИ молочных желез (по показаниям, в клинике)",
  "осмотр и пальпация",
  "маммография (по направлению и показаниям)",
  "лабораторные анализы по показаниям",
  "консультация онколога по результатам обследований",
];

const faqItems = [
  {
    q: "Когда обращаться к маммологу или онкологу?",
    a: "Обращайтесь при уплотнениях, боли, выделениях из соска, изменениях кожи груди, а также для профилактического осмотра. По результатам обследований врач может рекомендовать консультацию онколога.",
  },
  {
    q: "Какие приёмы доступны по прейскуранту?",
    a: "В клинике можно записаться на приём врача-онколога, диспансерный приём онколога и диспансерный приём врача-детского онколога. Стоимость указана в актуальном прайсе на сайте.",
  },
  {
    q: "Как записаться на приём?",
    a: "Запись через онлайн-регистратуру, форму на сайте или по телефону клиники. Администратор подберёт удобное время.",
  },
];
...
function Breadcrumbs() {
  return (
    <nav className="flex text-sm text-gray-500" aria-label="Breadcrumb">
      ...
      <span className="mx-2">/</span>
      <span className="text-gray-800">Маммология</span>
    </nav>
  );
}
...
export default function MammologyClient({ services, priceFrom }: Props) {
        ...
        >
          <div>
            <h1 className="text-2xl font-bold leading-tight text-gray-900 md:text-4xl">
              Маммология в Голицыно — диагностика и консультации в «Альтамед
              Голицыно»
            </h1>
            <p className="mt-4 text-base leading-relaxed text-gray-600 md:text-lg">
              Диагностика заболеваний молочных желез, консультации онколога и
              детского онколога. Помогаем разобраться с жалобами, подобрать
              обследования и план наблюдения.
            </p>
